...
object ImageUtils {
    ...
    init {
        try {
            ...
            LOGGER.w(TAG, "Native library not found, native RGB -> YUV conversion may be unavailable.")
        }
    ...
    fun saveBitmap(bitmap: Bitmap, filename: String = "preview.png") {
        ...
        LOGGER.i(TAG, "Saving ${bitmap.width} ${bitmap.height} bitmap to $root.")
        val myDir = File(root)

        if (!myDir.mkdirs()) {
            LOGGER.i(TAG, "Make dir failed")
        }
    ...
    fun convertYUV420SPToARGB8888(
        ...
        if (useNativeConversion) {
            try {
                ...
                LOGGER.w(
                    TAG, "Native YUV420SP -> RGB implementation not found, falling back to Java implementation"
                )
    ...
    fun convertYUV420ToARGB8888(
        ...
        if (useNativeConversion) {
            try {
                ...
                LOGGER.w(
                    TAG, "Native YUV420 -> RGB implementation not found, falling back to Java implementation"
                )
                useNativeConversion = false
            }
    ...
    fun getTransformationMatrix(
        ...
        if (applyRotation != 0) {
            if (applyRotation % 90 != 0) {
                LOGGER.w(TAG, "Rotation of $applyRotation % 90 != 0")
            }
